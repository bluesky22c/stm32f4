...
* lead code : 9ms ON    4.5ms OFF  = 13.5ms
* off   code : 2.25ms
* on   code : 1.12ms
*******************************************************************************/
void IRemoCon_ISR()
{
  //CallTrace(TRACE_DRV_HIGH, "IR %d\n\r",ir_count_100us);
  ...
  { 
	  case ready :
	  	...
	  	break;
	  case lead :	
		// between falling edge (8.5ms~140ms)
		// then, lead code!!!
		if((ir_count_100us >= 85) && (ir_count_100us < 140)){	
			  state = code; 								    
			  bit_count = 0;   
			  temp = 0;
	...
	case code : 											
		 ...
		 if((ir_count_100us < 15)){ // dummy time  1.5 ms 
